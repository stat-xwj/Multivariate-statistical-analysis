#  多维数据可视化
对于低维数据而言，可视化方法一般有以下几种：柱状图、散点图、饼图、折线图、直方图、密度图、箱线图、热力图、地图等。多维数据可视化的思路分为以下三种：
- 单个维度独立展示；
- 多个维度交互展示；
- 将多维数据映射到低维数据；
## 0. 数据集及数据说明
本文基于R语言对uci机器学习网站(https://archive.ics.uci.edu/ml/index.php)中葡萄酒数据进行可视化分析，主要用到ggplot2、GGally等包，完整数据及代码见文末，另附部分SAS代码见文末。该数据共分为红葡萄酒和白葡萄酒两类共6497条样本，均包含二氧化硫浓度、PH值、葡萄酒品质等12个变量。
```R
...
## 3. 将多维数据映射到低维数据
### 3.1 PCA
利用PCA将多维数据降维到低维数据，最终选择两个主成分，观察主成分表达式可以得出这两个主成分分别代表游离二氧化硫和总二氧化硫的浓度、游离二氧化硫相对于总二氧化硫的浓度，这也与上述热力图的结果一致，分别作出两个type下作出两个主成分的散点图：
...
完整代码（R+SAS）及数据见：https://github.com/stat-xwj/Multivariate-statistical-analysis/tree/master/visualization
